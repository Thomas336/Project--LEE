...
if CLIENT then
	...
	language.Add("Tool.wire_luagate.desc", "Spawns an Lua Gate for use with the wire system.")
	language.Add("sboxlimit_wire_luagates", "You've hit the Lua Gate limit!")
...
TOOL.MaxLimitName = "wire_luagates"
WireToolSetup.BaseLang()

if SERVER then
	CreateConVar('sbox_maxwire_luagates', 20)
	...
	function TOOL:Reload(trace)
		...
		if IsValid(trace.Entity) and trace.Entity:GetClass() == "gmod_wire_luagate" then
			trace.Entity:Reset()
			return true
		else
			return false
	...
	util.AddNetworkString("WireLuaGate_OpenEditor")
	function TOOL:RightClick(trace)
		...
		net.Start("WireLuaGate_OpenEditor") net.Send(player)
		return false
	end
	...
	function WireLib.LuaGateUpload( ply, target, filepath )
		...
		net.Start("wire_luagate_tool_upload")
	...
	function TOOL:Download(ply, ent)
		WireLib.LuaGateDownload(ply, ent, nil, true)
	end
	...
	-- --------------------------------------------------------------------------------------------------------------------------
	-- --------------------------------------------------------------------------------------------------------------------------
	util.AddNetworkString("wire_luagate_tool_upload")
	util.AddNetworkString("wire_luagate_editor_status")
	util.AddNetworkString("wire_luagate_download")
	util.AddNetworkString("wire_luagate_download_wantedfiles")
	util.AddNetworkString("wire_luagate_download_wantedfiles_list")
	util.AddNetworkString("wire_luagate_upload")
	util.AddNetworkString("wire_luagate_progress")

	-- ------------------------------------------------------------
	-- Serverside Send
	-- ------------------------------------------------------------
	function WireLib.LuaGateDownload(ply, targetEnt, wantedfiles, uploadandexit)
		if not IsValid(targetEnt) or targetEnt:GetClass() ~= "gmod_wire_luagate" then
			WireLib.AddNotify(ply, "Invalid Lua gate specified.", NOTIFY_ERROR, 7, NOTIFYSOUND_DRIP3)
			return
		end
		...
		if not hook.Run( "CanTool", ply, WireLib.dummytrace(targetEnt), "wire_luagate") then
			WireLib.AddNotify(ply, "You're not allowed to download from this Gate (ent index: " .. targetEnt:EntIndex() .. ").", NOTIFY_ERROR, 7, NOTIFYSOUND_DRIP3)
			return
		end

		local main = targetEnt:GetCode()
		--if not includes or not next(includes) then -- There are no includes
		local datastr = WireLib.von.serialize({ { targetEnt.MyName, main } })
		...
		for i = 1, #datastr, 64000 do
			timer.Simple( n, function()
				...
				net.Start("wire_luagate_download")
		...
		--[[elseif not wantedfiles then
			...
			net.Start("wire_luagate_download_wantedfiles_list")
			net.WriteEntity(targetEnt)
			net.WriteBit(uploadandexit or false)
	...
	local wantedfiles = {}
	net.Receive("wire_luagate_download_wantedfiles", function(len, ply)
		local toent = net.ReadEntity()
		local uploadandexit = net.ReadBit() ~= 0
		local numpackets = net.ReadUInt(16)

		if not IsValid(toent) or toent:GetClass() ~= "gmod_wire_luagate" then
			WireLib.AddNotify(ply, "Invalid entity specified to wire_luagate_download_wantedfiles. Download aborted.", NOTIFY_ERROR, 7, NOTIFYSOUND_DRIP3)
			return
		end
		...
		if numpackets <= #wantedfiles[ply] then
			...
			wantedfiles[ply] = nil
			if not ok then
				WireLib.AddNotify(ply, "Lua Gate download failed! Error message:\n" .. ret, NOTIFY_ERROR, 7, NOTIFYSOUND_DRIP3)
				print("Lua Gate download failed! Error message:\n" .. ret)
				return
			end

			WireLib.LuaGateDownload(ply, toent, ret, uploadandexit)
		end
	end)
	...
	local uploads = {}
	local upload_ents = {}
	net.Receive("wire_luagate_upload", function(len, ply)
		...
		if (not IsValid(toent) or toent:GetClass() ~= "gmod_wire_luagate") then
			if uploads[ply] then -- this is to prevent notification spam due to the net library automatically limiting its own transfer rate so that the messages arrive late
				uploads[ply] = nil
				upload_ents[ply] = nil
				WireLib.AddNotify(ply, "Invalid Lua gate specified. Upload aborted.", NOTIFY_ERROR, 7, NOTIFYSOUND_DRIP3)
			end
			return
		end

		if not hook.Run( "CanTool", ply, WireLib.dummytrace( toent ), "wire_luagate" ) then
			WireLib.AddNotify(ply, "You are not allowed to upload to the target Lua gate. Upload aborted.", NOTIFY_ERROR, 7, NOTIFYSOUND_DRIP3)
			return
		end
		...
		if numpackets <= #uploads[ply] then
			...
			if not ok then
				WireLib.AddNotify(ply, "Lua Gate upload failed! Error message:\n" .. ret, NOTIFY_ERROR, 7, NOTIFYSOUND_DRIP3)
				print("Lua Gate upload failed! Error message:\n" .. ret)
				return
			end
			...
			local filepath = ret[2] --CHECKME: Changed idx to 2 because no includes

			toent:LoadCode(code, filepath, ply)
		end
	end)
	...
	concommand.Add("wire_luagate_forcehalt", function(player, command, args)
		local E2 = tonumber(args[1])
		if not E2 then return end
		E2 = Entity(E2)
		if not IsValid(E2) or E2:GetClass() ~= "gmod_wire_luagate" then return end
		if canhas(player) then return end
		if E2.error then return end
		if hook.Run( "CanTool", player, WireLib.dummytrace( E2 ), "wire_luagate", "halt execution" ) then
			E2:PCallHook("destruct")
			E2:Error("Execution halted (Triggered by: " .. player:Nick() .. ")", "Execution halted")
			if E2.player ~= player then
				WireLib.AddNotify(player, "Gate halted.", NOTIFY_GENERIC, 5, math.random(1, 5))
				player:PrintMessage(HUD_PRINTCONSOLE, "Gate halted.")
			end
		else
	...
	concommand.Add("wire_luagate_requestcode", function(player, command, args)
		local E2 = tonumber(args[1])
		if not E2 then return end
		E2 = Entity(E2)
		if canhas(player) then return end
		if not IsValid(E2) or E2:GetClass() ~= "gmod_wire_luagate" then return end
		if hook.Run( "CanTool", player, WireLib.dummytrace( E2 ), "wire_luagate", "request code" ) then
			WireLib.LuaGateDownload(player, E2)
	...
	concommand.Add("wire_luagate_reset", function(player, command, args)
		local E2 = tonumber(args[1])
		if not E2 then return end
		E2 = Entity(E2)
		if not IsValid(E2) or E2:GetClass() ~= "gmod_wire_luagate" then return end
		if canhas(player) then return end
		if hook.Run( "CanTool", player, WireLib.dummytrace( E2 ), "wire_luagate", "reset" ) then
			if E2.context.data.last or E2.first then return end

			E2:Reset()

			WireLib.AddNotify(player, "Gate reset.", NOTIFY_GENERIC, 5, math.random(1, 4))
			player:PrintMessage(HUD_PRINTCONSOLE, "Gate reset.")
			if E2.player ~= player then
	...
	util.AddNetworkString( "wire_luagate_sync_ops" )
	concommand.Add("wire_luagate_ops_sync", function(player,command,args)
		...
		if next( players_synced ) and not timer.Exists( "wire_luagate_ops_sync" ) then
		
			timer.Create( "wire_luagate_ops_sync",0.2,0,function()
				local plys = {}
				...
				if not next( players_synced ) then
					timer.Remove( "wire_luagate_ops_sync" )
				end
			
				local E2s = ents.FindByClass("gmod_wire_luagate")

				net.Start( "wire_luagate_sync_ops" )
					net.WriteInt( #E2s, 16 )
					for i=1,#E2s do
		...
		elseif not next( players_synced ) and timer.Exists( "wire_luagate_ops_sync" ) then
			timer.Remove( "wire_luagate_ops_sync" )
		end
...
elseif CLIENT then
	...
	-- Client part
	------------------------------------------------------
	net.Receive( "wire_luagate_sync_ops", function( len )
	...
	local function next_queue()
		...
		LuaGateSetProgress( nil, queue_progress )
	...
	upload_queue = function(first)
		...
		local queue_progress = (queue_max > 1 and (1-((#queue-1) / queue_max)) * 100 or nil)
		LuaGateSetProgress(1, queue_progress)
		...
		for i = 1, #datastr, 64000 do
			timer.Simple( delay, function()
				...
				local queue_progress = (queue_max > 1 and (1-((#queue-1) / queue_max)) * 100 or nil)
				LuaGateSetProgress( packet / numpackets * 100, queue_progress )
				
				net.Start("wire_luagate_upload")
	...
	function WireLib.LuaGateUpload(targetEnt, code, filepath)
		...
		for i=1,#queue do
			if queue[i].targetEnt == targetEnt then
				WireLib.AddNotify("You're already uploading that gate!", NOTIFY_ERROR, 7, NOTIFYSOUND_ERROR1)
				return
			end
		end

		if not code and not wire_luagate_editor then return end -- If the player leftclicks without opening the editor or cpanel (first spawn)
		code = code or wire_luagate_editor:GetCode()
		filepath = filepath or wire_luagate_editor:GetChosenFile()
		--local err, includes

		--[[if e2_function_data_received then
			err, includes = wire_luagate_validate(code)
			if err then
				WireLib.AddNotify(err, NOTIFY_ERROR, 7, NOTIFYSOUND_ERROR1)
				return
			end
		else
			WireLib.AddNotify("The Lua Gate function data has not been transferred to the client yet;\n uploading the E2 to the server for validation.\nNote that any includes will not be sent. You must wait for the function data to finish\n transmitting before you are able to use includes.", NOTIFY_ERROR, 14, NOTIFYSOUND_DRIP3)
		...
		datastr = E2Lib.encode(WireLib.von.serialize({ code, filepath })) --CHECKME: arg 2 was {}
		--end
	...
	net.Receive("wire_luagate_tool_upload", function(len, ply)
	...
	local current_ent
	net.Receive("wire_luagate_download", function(len)
		local ent = net.ReadEntity()
		
		if IsValid( current_ent ) and IsValid( ent ) and ent ~= current_ent then
			-- different LG, reset buffer
			buffer = ""
			count = 0
		end
		...
		LuaGateSetProgress(count / numpackets * 100, nil, "Downloading")
		if numpackets <= count then
			...
			if not wire_luagate_editor then initLGEditor() end

			if uploadandexit then
				--wire_luagate_editor.chip = ent
			end

			--[[if files[2] and next(files[2]) then
				for k, v in pairs(files[2]) do
					wire_luagate_editor:Open(k, v)
				end
			end]]--

			wire_luagate_editor:Open(name, main)
			timer.Create("wire_luagate_reset_progress", 0.75, 1, LuaGateSetProgress)
		end
	end)

	net.Receive("wire_luagate_download_wantedfiles_list", function(len)
		local ent = net.ReadEntity()
		local uploadandexit = net.ReadBit() ~= 0
		...
		local ok, ret = pcall(WireLib.von.deserialize, buffer)
		if not ok then
			WireLib.AddNotify(ply, "Lua Gate file list download failed! Error message:\n" .. ret, NOTIFY_ERROR, 7, NOTIFYSOUND_DRIP3)
			print("Lua Gate file list download failed! Error message:\n" .. ret)
			return
		end
		...
		function ok:DoClick()
			...
			local numpackets = math.ceil(#datastr / 64000)
			for i = 1, #datastr, 64000 do
				net.Start("wire_luagate_download_wantedfiles")
				net.WriteEntity(ent)
	...
	function TOOL.BuildCPanel(panel)
		...
		WireToolHelpers.MakeModelSizer(panel, "wire_luagate_modelsize")
		--[[
		...
		panel:AddControl("MaterialGallery", {
			...
			Rows = 2,
			Stretch = false,
			ConVar = "wire_luagate_select",
			Options = {
				["Modern"] = { wire_luagate_select = "Modern", Value = "Modern", Material = "beer/wiremod/gate_e2", wire_luagate_model = "models/beer/wiremod/gate_e2.mdl" },
				["Expression"] = { wire_luagate_select = "Expression", Value = "Expression", Material = "models/expression 2/exprssn", wire_luagate_model = "models/expression 2/cpu_expression.mdl" },
				["Microchip"] = { wire_luagate_select = "Microchip", Value = "Microchip", Material = "models/expression 2/mcrochp", wire_luagate_model = "models/expression 2/cpu_microchip.mdl" },
				["Interface"] = { wire_luagate_select = "Interface", Value = "Interface", Material = "models/expression 2/intrfce", wire_luagate_model = "models/expression 2/cpu_interface.mdl" },
				["Controller"] = { wire_luagate_select = "Controller", Value = "Controller", Material = "models/expression 2/cntrllr", wire_luagate_model = "models/expression 2/cpu_controller.mdl" },
				["Processor"] = { wire_luagate_select = "Processor", Value = "Processor", Material = "models/expression 2/prcssor", wire_luagate_model = "models/expression 2/cpu_processor.mdl" },
			}
		})

		if (wire_luagate_editor == nil) then initLGEditor() end

		local FileBrowser = vgui.Create("wire_expression2_browser", panel)
		FileBrowser.OpenOnSingleClick = wire_luagate_editor
		...
		function FileBrowser:OnFileOpen(filepath, newtab)
			wire_luagate_editor:Open(filepath, nil, newtab)
		end

		local OpenEditor = panel:Button("Open Editor")
		OpenEditor.DoClick = function(button)
			wire_luagate_editor:Open()
		end

		local NewScript = panel:Button("New Script")
		NewScript.DoClick = function(button)
			wire_luagate_editor:Open()
			wire_luagate_editor:NewScript()
		end
	end

	function initEditor()
		wire_luagate_editor = vgui.Create("Expression2EditorFrame")
		wire_luagate_editor:Setup("Lua Gate Editor", "luagate", "luagate")
	end

	function openEditor()
		if (wire_luagate_editor == nil) then initEditor() end
		wire_luagate_editor:Open()
	end

	net.Receive("WireLuaGate_OpenEditor", openEditor)

	--[[
	  Lua Gate Tool Screen for Garry's Mod
	...
	surface.CreateFont("LuaGateToolScreenFont", fontTable)
	fontTable.size = 30
	surface.CreateFont("LuaGateToolScreenSubFont", fontTable)
	...
	function LuaGateSetName(n)
		...
		surface.SetFont("LuaGateToolScreenSubFont")
	...
	function LuaGateSetProgress(p, p2, w)
		what = w or "Uploading"
	...
	function TOOL:DrawToolScreen(width, height)
		...
		surface.SetFont("LuaGateToolScreenFont")
		...
		if percent or percent2 then		
			...
			DrawTextOutline(what, "LuaGateToolScreenFont", 128, 128, Color(224, 224, 224, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, Color(0, 0, 0, 255), 4)
		...
		elseif name then
			DrawTextOutline("Lua Gate", "LuaGateToolScreenFont", 128, 128, Color(224, 224, 224, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, Color(0, 0, 0, 255), 4)
			DrawTextOutline(name, "LuaGateToolScreenSubFont", 128, 128 + (h + h2) / 2 - 4, Color(224, 224, 224, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, Color(0, 0, 0, 255), 4)
		end
	end
end
...
if SERVER then
	...
	local wire_luagate_event = {}

	concommand.Add("wire_luagate_event", function(ply, command, args)
		local handler = wire_luagate_event[args[1]]
	...
	function wire_luagate_event.editor_open(ply, args)
		net.Start("wire_luagate_editor_status")
		net.WriteEntity(ply)
		net.WriteBit(true)
		net.Broadcast()
	end

	function wire_luagate_event.editor_close(ply, args)
		net.Start("wire_luagate_editor_status")
		net.WriteEntity(ply)
		net.WriteBit(false)
		net.Broadcast()
...
elseif CLIENT then
	...
	local busy_players = {}
	hook.Add("EntityRemoved", "wire_luagate_busy_animation", function(ply)
		busy_players[ply] = nil
	end)

	local emitter = ParticleEmitter(vector_origin)

	net.Receive("wire_luagate_editor_status", function(len)
	...
	timer.Create("wire_luagate_editor_status", 1, 0, function()
		rolldelta = -rolldelta
